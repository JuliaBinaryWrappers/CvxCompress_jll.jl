...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import CvxCompress_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("CvxCompress_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "CvxCompress_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"CvxCompress", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "CvxCompress_jll", "src", "CvxCompress_jll.jl"))
        @info("CvxCompress_ll dev'ed out to /depot/dev/CvxCompress_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("2493bdd5-554a-52e1-bea5-dc84d30aecb9"))
platforms = [Pkg.Artifacts.unpack_platform(e, "CvxCompress", artifacts_toml) for e in artifacts["CvxCompress"]]
...
if best_platform === nothing
    @debug("Unable to load CvxCompress; unsupported platform $(triplet(platform_key_abi()))")
else
